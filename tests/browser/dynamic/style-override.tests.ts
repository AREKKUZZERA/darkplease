...
describe('Style override', () => {
    ...
    it('should restore override', async () => {
        ...
        await expect(pageUtils.evaluateScript(async () => {
            ...
            return (style.nextSibling as HTMLStyleElement).classList.contains('darkplease--sync');
        })).resolves.toBe(true);
    });
    ...
    it('should remove override', async () => {
        ...
        await expect(pageUtils.evaluateScript(async () => {
            ...
            style.remove();
            await new Promise((resolve) => setTimeout(resolve));
            return sibling.isConnected && !((sibling as HTMLStyleElement).classList.contains('darkplease--sync'));
        })).resolves.toBe(false);
    });
    ...
    it('should react to updated style', async () => {
        ...
        })).resolves.toBe('html { background-color: var(--darkplease-background-ffc0cb, #590010); }');
    });
    ...
    it('should react to a new style', async () => {
        ...
        await expect(pageUtils.evaluateScript(async () => {
            ...
            await new Promise((resolve) => setTimeout(resolve));
            return (styleElement.nextSibling as HTMLStyleElement).sheet.cssRules.length === 2 && (styleElement.nextSibling as HTMLStyleElement).classList.contains('darkplease--sync');
        })).resolves.toBe(true);
    });
