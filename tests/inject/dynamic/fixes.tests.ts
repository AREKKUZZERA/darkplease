...
afterEach(() => {
    removeDynamicTheme();
    container.innerHTML = '';
    removeNode(document.head.querySelector('meta[name="darkplease-lock"]'));
});

describe('FIXES', () => {
    it('should add custom attributes to root element', () => {
        createOrUpdateDynamicTheme(DEFAULT_THEME, null, false);
        expect(document.documentElement.getAttribute(`data-darkplease-mode`)).toBe('dynamic');
        expect(document.documentElement.getAttribute('data-darkplease-scheme')).toBe('dark');

        createOrUpdateDynamicTheme({...DEFAULT_THEME, mode: FilterMode.light}, null, false);
        expect(document.documentElement.getAttribute('data-darkplease-scheme')).toBe('dimmed');
    });
    ...
    it('should ignore styling when darkplease-lock detected', async () => {
        document.head.innerHTML = '<meta name="darkplease-lock">',
        container.innerHTML = multiline(
            '<style>',
            '    body {',
    ...
    it('should ignore styling when delayed darkplease-lock detected', async () => {
        container.innerHTML = multiline(
            '<style>',
            '    body {',
            '        background-color: pink !important;',
        ...
        metaElement.name = 'darkplease-lock';
