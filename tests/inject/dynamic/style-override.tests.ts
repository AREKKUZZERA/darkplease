...
describe('STYLE ELEMENTS', () => {
    it('should fill CSSStyleSheet with overridden rules', () => {
        ...
        expect((override.cssRules[0] as CSSStyleRule).style.getPropertyValue('background-color')).toBe('var(--darkplease-background-ffffff, #000000)');
        expect((override.cssRules[1] as CSSStyleRule).selectorText).toBe('h1');
        expect((override.cssRules[1] as CSSStyleRule).style.getPropertyValue('color')).toBe('var(--darkplease-text-000000, #ffffff)');
    });
    ...
    it('should restore override', async () => {
        ...
        await timeout(0);
        expect((style.nextSibling as HTMLStyleElement).classList.contains('darkplease--sync')).toBe(true);
        expect(getComputedStyle(container.querySelector('h1')).color).toBe('rgb(141, 141, 141)');
        expect(getComputedStyle(container.querySelector('h1 strong')).color).toBe('rgb(255, 26, 26)');
    });
    ...
    it('should remove override', async () => {
        ...
        expect(document.querySelector('.darkplease--sync')).toBe(null);
        expect(getComputedStyle(container.querySelector('h1')).color).toBe('rgb(255, 255, 255)');
    ...
    it('should react to updated style', async () => {
        ...
        await timeout(0);
        expect((style.nextSibling as HTMLStyleElement).sheet.cssRules[0].cssText).toBe('body { background-color: var(--darkplease-background-ffc0cb, #320009); }');
        expect(getComputedStyle(container.querySelector('h1')).color).toBe('rgb(141, 141, 141)');
        expect(getComputedStyle(container.querySelector('h1 strong')).color).toBe('rgb(255, 26, 26)');
    ...
    it('should react to a new style', async () => {
        ...
        expect(nextSibling.classList.contains('darkplease--sync')).toBe(true);
