...
export async function getActiveTab(): Promise<chrome.tabs.Tab | null> {
    ...
    let tab = (await queryTabs({
        ...
        // Explicitly exclude DARK PLEASE!'s Dev Tools and other special windows from the query
        windowType: 'normal',
    }))[0];
    if (!tab) {
        tab = (await queryTabs({
    ...
    if (!tab) {
        ...
        // When DARK PLEASE!'s DevTools are open, last focused window might be the DevTools window
