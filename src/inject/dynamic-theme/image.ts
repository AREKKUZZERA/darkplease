...
document.addEventListener('__darkplease__inlineScriptsAllowed', () => canUseProxy = true, {once: true});
...
export async function requestBlobURLCheck(): Promise<void> {
    ...
    await new Promise<void>((resolve) => {
        document.addEventListener('__darkplease__blobURLCheckResponse', (e: CustomEvent) => {
            isBlobURLSupported = e.detail.blobURLAllowed;
            resolve();
            blobURLCheckAwaiters.forEach((r) => r());
            blobURLCheckAwaiters.splice(0);
        }, {once: true});
        document.dispatchEvent(new CustomEvent('__darkplease__blobURLCheckRequest'));
    });
}
...
export function getFilteredImageURL({dataURL, width, height, useViewBox}: ImageDetails, theme: Theme): string {
    ...
    const svg = [
        ...
        '<defs>',
        '<filter id="darkplease-image-filter">',
        `<feColorMatrix type="matrix" values="${matrix}" />`,
        '</filter>',
        '</defs>',
        `<image width="${width}" height="${height}" filter="url(#darkplease-image-filter)" xlink:href="${dataURL}" />`,
        '</svg>',
    ].join('');
