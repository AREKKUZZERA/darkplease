...
document.addEventListener('__darkplease__inlineScriptsAllowed', () => canUseSheetProxy = true, {once: true});
...
export function createAdoptedStyleSheetOverride(node: Document | ShadowRoot): AdoptedStyleSheetManager {
    ...
    function render(theme: Theme, ignoreImageAnalysis: string[]) {
        ...
        for (let i = node.adoptedStyleSheets.length - 1; i >= 0; i--) {
            ...
            const prepareSheet = () => {
                ...
                override.insertRule('#__darkplease__adoptedOverride {}');
    ...
    function watch(callback: (sheets: CSSStyleSheet[]) => void) {
        ...
        addSheetChangeEventListener('__darkplease__adoptedStyleSheetsChange', onAdoptedSheetsChange);
        addSheetChangeEventListener('__darkplease__adoptedStyleSheetChange', onAdoptedSheetsChange);
        addSheetChangeEventListener('__darkplease__adoptedStyleDeclarationChange', onAdoptedSheetsChange);
