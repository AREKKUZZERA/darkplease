...
export function injectProxy(enableStyleSheetsProxy: boolean, enableCustomElementRegistryProxy: boolean): void {
    document.dispatchEvent(new CustomEvent('__darkplease__inlineScriptsAllowed'));
    ...
    documentEventListener('__darkplease__cleanUp', cleanUp);
    documentEventListener('__darkplease__disableConflictingPlugins', disableConflictingPlugins);
    ...
    function isDRElement(element?: Element) {
        return element?.classList?.contains('darkplease');
    }
    ...
    const updateSheetEvent = new CustomEvent('__darkplease__updateSheet');
    const adoptedSheetChangeEvent = new CustomEvent('__darkplease__adoptedStyleSheetChange');
    const shadowDomAttachingEvent = new CustomEvent('__darkplease__shadowDomAttaching', {bubbles: true});
    ...
    function resolveCustomElement(tag: string) {
        customElements.whenDefined(tag).then(() => {
            document.dispatchEvent(new CustomEvent('__darkplease__isDefined', {detail: {tag}}));
        });
    }

    documentEventListener('__darkplease__addUndefinedResolver', (e: CustomEvent<{tag: string}>) => resolveCustomElement(e.detail.tag));
    ...
    function checkBlobURLSupport() {
        if (blobURLAllowed != null) {
            document.dispatchEvent(new CustomEvent('__darkplease__blobURLCheckResponse', {detail: {blobURLAllowed}}));
            return;
        }
    ...
    documentEventListener('__darkplease__blobURLCheckRequest', checkBlobURLSupport);
    ...
    if (!(__FIREFOX_MV2__ || __THUNDERBIRD__)) {
        ...
        const adoptedSheetsChangeEvent = new CustomEvent('__darkplease__adoptedStyleSheetsChange');
        ...
        const isDRAdoptedSheetOverride = (sheet: CSSStyleSheet) => {
            ...
            if (sheet.cssRules.length > 0 && sheet.cssRules[0].cssText.startsWith('#__darkplease__adoptedOverride')) {
        ...
        const adoptedDeclarationChangeEvent = new CustomEvent('__darkplease__adoptedStyleDeclarationChange');
    ...
    if (__FIREFOX_MV2__ || __THUNDERBIRD__) {
        ...
        const sendSourceStyles = (sheets: NodeSheet[]) => {
            ...
            const event = new CustomEvent('__darkplease__adoptedStyleSheetsChange', data);
        ...
        documentEventListener('__darkplease__startAdoptedStyleSheetsWatcher', () => {
            proxyAdoptedStyleSheets();
            proxyStyleDeclaration();

            documentEventListener('__darkplease__adoptedStyleSheetCommands', commandsListener);
