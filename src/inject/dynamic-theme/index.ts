...
function createOrUpdateStyle(className: string, root: ParentNode = document.head || document): HTMLStyleElement {
    ...
    if (element) {
        ...
        element.classList.add('darkplease');
...
function createStaticStyleOverrides() {
    const fallbackStyle = createOrUpdateStyle('darkplease--fallback', document);
    fallbackStyle.textContent = getModifiedFallbackStyle(theme!, {strict: true});
    injectStaticStyle(fallbackStyle, null, 'fallback');

    const userAgentStyle = createOrUpdateStyle('darkplease--user-agent');
    ...
    const textStyle = createOrUpdateStyle('darkplease--text');
    ...
    const invertStyle = createOrUpdateStyle('darkplease--invert');
    ...
    const inlineStyle = createOrUpdateStyle('darkplease--inline');
    inlineStyle.textContent = getInlineOverrideStyle();
    injectStaticStyle(inlineStyle, invertStyle, 'inline');

    const variableStyle = createOrUpdateStyle('darkplease--variables');
    ...
    variableStyle.textContent = [
        `:root {`,
        `   --darkplease-neutral-background: ${neutralBackgroundColor};`,
        `   --darkplease-neutral-text: ${neutralTextColor};`,
        `   --darkplease-selection-background: ${selectionColors?.backgroundColorSelection ?? 'initial'};`,
    ...
    document.dispatchEvent(new CustomEvent('__darkplease__cleanUp'));
    if (__CHROMIUM_MV3__) {
        // Notify the dedicated injector of the data.
        document.dispatchEvent(new CustomEvent('__darkplease__stylesheetProxy__arg', {detail: {enableStyleSheetsProxy, enableCustomElementRegistryProxy}}));
    } else {
        const proxyScript = createOrUpdateScript('darkplease--proxy');
        proxyScript.append(`(${injectProxy})(${enableStyleSheetsProxy}, ${enableCustomElementRegistryProxy})`);
        document.head.insertBefore(proxyScript, rootVarsStyle.nextSibling);
        proxyScript.remove();
    }

    const overrideStyle = createOrUpdateStyle('darkplease--override');
...
function createShadowStaticStyleOverridesInner(root: ShadowRoot) {
    const inlineStyle = createOrUpdateStyle('darkplease--inline', root);
    inlineStyle.textContent = getInlineOverrideStyle();
    root.insertBefore(inlineStyle, root.firstChild);
    const overrideStyle = createOrUpdateStyle('darkplease--override', root);
    overrideStyle.textContent = fixes && fixes.css ? replaceCSSTemplates(fixes.css) : '';
    root.insertBefore(overrideStyle, inlineStyle.nextSibling);

    const invertStyle = createOrUpdateStyle('darkplease--invert', root);
    if (fixes && Array.isArray(fixes.invert) && fixes.invert.length > 0) {
        invertStyle.textContent = [
            `${fixes.invert.join(', ')} {`,
            `    filter: ${getCSSFilterValue({
...
function delayedCreateShadowStaticStyleOverrides(root: ShadowRoot): void {
    const observer = new MutationObserver((mutations, observer) => {
        ...
        // Do not make any changes unless DARK PLEASE!'s fixes have been removed
        for (const {type, removedNodes} of mutations) {
            if (type === 'childList') {
                for (const {nodeName, className} of removedNodes as any) {
                    if (nodeName === 'STYLE' && ['darkplease darkplease--inline', 'darkplease darkplease--override', 'darkplease darkplease--invert'].includes(className)) {
...
function cleanFallbackStyle() {
    const fallback = (
        staticStyleMap.get(document.head)?.get('darkplease--fallback') ||
        staticStyleMap.get(document)?.get('darkplease--fallback') ||
        document.querySelector('.darkplease--fallback')
    );
    if (fallback) {
    ...
    if (isFirefox) {
        ...
        const onAdoptedCssChange = (e: CustomEvent) => {
            ...
            requestAnimationFrameOnce(getAdoptedStyleChangeToken(sheets[0].sheet), () => {
                document.dispatchEvent(new CustomEvent('__darkplease__adoptedStyleSheetCommands', {detail: JSON.stringify(response)}));
            });
        };

        document.addEventListener('__darkplease__adoptedStyleSheetsChange', onAdoptedCssChange);
        cleaners.push(() => document.removeEventListener('__darkplease__adoptedStyleSheetsChange', onAdoptedCssChange));

        document.dispatchEvent(new CustomEvent('__darkplease__startAdoptedStyleSheetsWatcher'));
    }
}
...
function createManager(element: StyleElement) {
    ...
    function loadingStart() {
        if (!isDOMReady() || !documentIsVisible()) {
            ...
            const fallbackStyle = createOrUpdateStyle('darkplease--fallback');
    ...
    function update() {
        ...
        if (__TEST__) {
            document.dispatchEvent(new CustomEvent('__darkplease__test__dynamicUpdateComplete'));
        }
    }
...
function addMetaListener() {
    metaObserver = new MutationObserver(() => {
        if (document.querySelector('meta[name="darkplease-lock"]')) {
...
function createDarkPleaseInstanceMarker() {
    const metaElement: HTMLMetaElement = document.createElement('meta');
    metaElement.name = 'darkplease';
    metaElement.content = INSTANCE_ID;
    document.head.appendChild(metaElement);
}

function isDRLocked() {
    return document.querySelector('meta[name="darkplease-lock"]') != null;
}

function isAnotherDarkPleaseInstanceActive() {
    const meta: HTMLMetaElement | null = document.querySelector('meta[name="darkplease"]');
    if (meta) {
    ...
    createDarkPleaseInstanceMarker();
    addMetaListener();
    return false;
}
...
function interceptOldScript({success, failure}: {success: () => void; failure: () => void}) {
    ...
    const oldMeta = document.head.querySelector('meta[name="darkplease"]') as HTMLMetaElement | null;
    if (!oldMeta || oldMeta.content === INSTANCE_ID) {
        return;
    }

    const lock = document.createElement('meta');
    lock.name = 'darkplease-lock';
...
function disableConflictingPlugins() {
    if (document.documentElement.hasAttribute('data-wp-dark-mode-preset')) {
        const disableWPDarkMode = () => {
            document.dispatchEvent(new CustomEvent('__darkplease__disableConflictingPlugins'));
...
function tryInvertChromePDF() {
    if (!document.body || !chrome.dom || !isExtensionContextValid()) {
        return;
    }
...
export function createOrUpdateDynamicThemeInternal(themeConfig: Theme, dynamicThemeFixes: DynamicThemeFix | null, iframe: boolean): void {
    ...
    const ready = () => {
        const success = () => {
            ...
            document.documentElement.setAttribute('data-darkplease-mode', 'dynamic');
            document.documentElement.setAttribute('data-darkplease-scheme', theme!.mode ? 'dark' : 'dimmed');
        ...
        if (isDRLocked()) {
            removeNode(document.querySelector('.darkplease--fallback'));
        } else if (isAnotherDarkPleaseInstanceActive()) {
            interceptOldScript({
                success,
                failure,
            });
...
function removeProxy() {
    document.dispatchEvent(new CustomEvent('__darkplease__cleanUp'));
    removeNode(document.head.querySelector('.darkplease--proxy'));
}
...
export function removeDynamicTheme(): void {
    ...
    document.documentElement.removeAttribute(`data-darkplease-scheme`);
    cleanDynamicThemeCache();
    removeNode(document.querySelector('.darkplease--fallback'));
    if (document.head) {
        const selectors = [
            '.darkplease--user-agent',
            '.darkplease--text',
            '.darkplease--invert',
            '.darkplease--inline',
            '.darkplease--override',
            '.darkplease--variables',
            '.darkplease--root-vars',
            'meta[name="darkplease"]',
        ];
    ...
    shadowRootsWithOverrides.forEach((root) => {
        removeNode(root.querySelector('.darkplease--inline'));
        removeNode(root.querySelector('.darkplease--override'));
    });
    shadowRootsWithOverrides.clear();
    forEach(styleManagers.keys(), (el) => removeManager(el));
    loadingStyles.clear();
    cleanLoadingLinks();
    forEach(document.querySelectorAll('.darkplease'), removeNode);
