...
document.addEventListener('__darkplease__inlineScriptsAllowed', () => {
    canOptimizeUsingProxy = true;
}, {once: true, passive: true});

const unhandledShadowHosts = new Set<Element>();

document.addEventListener('__darkplease__shadowDomAttaching', (e: CustomEvent) => {
...
async function customElementsWhenDefined(tag: string): Promise<void> {
    ...
    return new Promise<void>((resolve) => {
        ...
        if (window.customElements && typeof customElements.whenDefined === 'function') {
            ...
            document.dispatchEvent(new CustomEvent('__darkplease__addUndefinedResolver', {detail: {tag}}));
        } else {
...
export function unsubscribeFromDefineCustomElements(): void {
    ...
    document.removeEventListener('__darkplease__isDefined', handleIsDefined);
}
