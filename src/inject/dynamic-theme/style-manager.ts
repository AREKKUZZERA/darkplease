...
function hasValidLinkHref(link: HTMLLinkElement): boolean {
    const hrefAttr = link.getAttribute('href');
    if (!hrefAttr) {
        return false;
    }
    const href = hrefAttr.trim().toLowerCase();
    return href.length > 0 && href !== 'null';
}

export function cleanLoadingLinks(): void {
    rejectorsForLoadingLinks.forEach((reject) => reject());
...
export function manageStyle(element: StyleElement, {update, loadingStart, loadingEnd}: {update: () => void; loadingStart: () => void; loadingEnd: () => void}): StyleManager {
    ...
    if (inMode === 'next') {
        ...
        while ((next = next.nextElementSibling) && next.matches('.darkplease')) {
            prevStyles.push(next as HTMLStyleElement);
        }
        syncStyle = prevStyles.find((el) => el.matches('.darkplease--sync') && !syncStyleSet.has(el)) || null;
    }
    ...
    function createSyncStyle() {
        ...
        syncStyle.classList.add('darkplease');
        syncStyle.classList.add('darkplease--sync');
    ...
    let attemptedAsyncOnSecondRound = false;
    ...
    function details(options: detailsArgument) {
        ...
        if (!rules) {
            if (options.secondRound && attemptedAsyncOnSecondRound) {
            ...
            if (options.secondRound) {
                attemptedAsyncOnSecondRound = true;
            }
            loadingStart();
...
async function linkLoading(link: HTMLLinkElement, loadingId: number) {
    return new Promise<void>((resolve, reject) => {
        if (!hasValidLinkHref(link)) {
            reject(`Linkelement ${loadingId} doesn't have href.`);
            return;
        }
        ...
        let loadingPollId: ReturnType<typeof setInterval> | null = null;

        const settle = (setter: () => void) => {
            if (isSettled) {
                return;
            }
            isSettled = true;
        ...
        rejectorsForLoadingLinks.set(loadingId, (reason?: any) => {
            settle(() => {
                cleanUp();
                reject(reason);
            });
        ...
        loadingTimeoutId = setTimeout(() => {
            settle(() => {
                cleanUp();
                reject(`Linkelement ${loadingId} loading timed out. ${link.href}`);
            });
        }, 5000);
    });
}
...
function getCSSImportURL(importDeclaration: string) {
    ...
    const urlFunctionMatch = declaration.match(/^url\(([^)]*)\)/i);
    if (urlFunctionMatch) {
        return getCSSURLValue(`url(${urlFunctionMatch[1]})`);
    }

    const quotedURLMatch = declaration.match(/^((".*?")|('.*?'))/);
    if (quotedURLMatch) {
        return getCSSURLValue(quotedURLMatch[1]);
    }
