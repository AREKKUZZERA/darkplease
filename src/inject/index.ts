...
function sendMessage(message: MessageCStoBG | MessageCStoUI): true | undefined {
    ...
    try {
        if (__CHROMIUM_MV3__) {
            const sendMessageResult = chrome.runtime.sendMessage<MessageCStoBG | MessageCStoUI, MessageBGtoCS | 'unsupportedSender'>(message);
...
if (__PLUS__) {
    if (location.origin === HOMEPAGE_URL) {
        document.addEventListener('__darkplease_activate__', async (e: CustomEvent) => {
            const {email, key} = e.detail;
            const result = await activateTheme(email, key);
            document.dispatchEvent(new CustomEvent('__darkplease_activationResult__', {detail: {result}}));
        }, {once: true});
    }
}
