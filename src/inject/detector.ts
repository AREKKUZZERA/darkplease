...
function runCheck(callback: (hasDarkTheme: boolean) => void) {
    ...
    const drSheets = Array.from(document.styleSheets).filter((s) => (s.ownerNode as HTMLElement)?.classList.contains('darkplease'))
        .concat(
            Array.isArray(document.adoptedStyleSheets) ? Array.from(document.adoptedStyleSheets).filter(
                (s) => (s.cssRules?.[0] as CSSStyleRule)?.selectorText?.startsWith('#__darkplease')
            ) : [],
        );
