...
export default class TabManager {
    ...
    private static sendDocumentMessage(tabId: number, documentId: string, message: MessageBGtoCS, frameId: number) {
        if (frameId === 0) {
            ...
            if (themeMessageTypes.includes(message.type)) {
                ...
                const isActive = TabManager.tabs[tabId]?.[0]?.url?.startsWith('https://github.com/AREKKUZZERA/darkplease');
    ...
    // and not just that tab as DARK PLEASE! currently doesn't have per-tab operations,
