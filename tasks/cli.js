 ...
 *  2. Support building older versions of DARK PLEASE! and then inserting signatures into archives
 */
...
async function checkoutVersion(version, fixVulnerabilities) {
    ...
    const deps = /** @type {{audit: {auditReportVersion: number; metadata: {vulnerabilities: {total: number}}}}} */ (JSON.parse(await execute('npm audit fix --force --ignore-scripts --json')));
...
async function run() {
    ...
    if (version) {
        try {
            ...
            log.error(`Could not check out tag ${version}. ${message}`);
            return;
        }
    }
