...
async function ping(url) {
    try {
        ...
        const error = /** @type {Error & {cause?: string}} */ (err);
    ...
    for (const pattern of patterns) {
        ...
        if (HTTPS_GET) {
            ...
            if (status === 200) {
                ...
                log.ok(`${status} ${url}`);
                canClearPrevLine = true;
            } else if (typeof status === 'number' && (status > 200 && status <= 299) || status !== 404) {
                clearLineIfNeeded();
                log.warn(`${status} ${url}`);
                canClearPrevLine = false;
...
async function cleanConfig(title, filePath) {
    ...
    const urlsPerBlock = blocks.map((b) => {
        ...
        const urls = /** @type {string[]} */ (b.slice(urlStart, urlEnd).trim().split('\n'));
        patterns.push(...urls);
        return new Set(urls);
    });
