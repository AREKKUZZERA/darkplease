...
async function getOutdated() {
    return /** @type {Promise<Record<string, {latest: string}> | null>} */(new Promise((resolve, reject) => {
...
/**
 * @param {Record<string, {latest: string}>} outdated
...
async function main() {
    ...
    await command('mv darkplease.js darkplease-old.js');
    await command('mv darkplease.mjs darkplease-old.mjs');
    ...
    await command('diff darkplease-old.js darkplease.js');
    await command('diff darkplease-old.mjs darkplease.mjs');
