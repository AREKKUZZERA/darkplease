...
/** @typedef {import('./types').BuildTaskOptions} BuildTaskOptions */
...
async function run({release, debug, platforms, watch, log: logging, test, version}) {
    const regular = Object.values(PLATFORM).some((platform) => platform !== PLATFORM.API && platforms[platform]);
    if (release && regular) {
        await build({platforms, version, debug: false, watch: false, log: null, test: false});
    }
    if (debug && regular) {
        await build({platforms, version, debug, watch, log: logging, test});
    }
...
function getParams(args) {
    ...
    /** @type {LogLevel} */
    const logging = logWarn ? 'warn' : (logInfo ? 'info' : (logAssert ? 'assert' : null));
    const test = args.includes('--test');

    return {release, debug, platforms, watch, log: logging, test, version};
}
